...
def replace_customer_logo(
    ...
    """Replace all 'Customer Logo' placeholder shapes with an image.

    `image_url` must be a publicly reachable URL (e.g. served by the Flask
    app itself) so Google's Slides API servers can fetch it.
    ...
    for ph in logo_placeholders:
        ...
        try:
            ...
            if on_progress:
                on_progress(f"Logo replacement failed for '{ph}': {e}")
